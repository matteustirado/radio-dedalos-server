...
class GameController {
    ...
    static async getGameVoteCounts(request, response) {
        try {
            const { unit } = request.params;
            const voteCounts = await GameVoteModel.getVoteCounts(unit);
            console.log(`[GameController] Retornando contagem inicial para ${unit}:`, voteCounts);
            response.status(200).json(voteCounts);
        } catch (error) {
            console.error("Erro ao buscar contagem de votos inicial:", error);
            response.status(500).json({ message: 'Erro ao buscar contagem de votos.' });
        }
    }
